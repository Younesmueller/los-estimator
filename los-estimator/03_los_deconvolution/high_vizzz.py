...
class Visualizer():
    ...
    def _show(self,filename: str, fig: Optional[plt.Figure] = None):
...
class DeconvolutionPlots (Visualizer):
#   visualizer = DeconvolutionPlots(all_fit_results,series_data,params,vd)

    def __init__(self, all_fit_results: MultiSeriesFitResults, series_data: SeriesData, params: Params, vd: types.SimpleNamespace):
        super().__init__()
        
        # data
        self.vd: types.SimpleNamespace = vd
        ...
        self.figures_folder = vd.figures_folder
    ...
    def _ax_plot_prediction_error_window(self,ax,fr_series,distro,error_window_alpha=0.1):
        ...
        ax.set_xticks(self.vd.xtick_pos[::2])
        ax.set_xticklabels(self.vd.xtick_label[::2])
        ax.set_xlim(*self.vd.xlims)
        ax.grid(zorder=0)
    ...
    def superimpose_kernels(self, distro: Optional[Union[str, List[str]]] = None):
        ...
        for distro in distros:
            ...
            # plot real kernel
            r, = plt.plot(self.vd.real_los, color='black', label="Real")
    ...
    def generate_plots_for_run(self,show_plots:Optional[bool]=None,save_plots:Optional[bool]=None):
