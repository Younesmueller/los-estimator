...
class LOSEstimator:
    ...
    def visualize_results(self, save_plots: bool = True) -> Dict[str, str]:
        ...
        if save_plots and 'figures_folder' in self.results.metadata:
            ...
            # Get additional data for enhanced visualizations
            real_los = None
            df_mutant = None
            if self.data:
                real_los = self.data.get('real_los')
                df_mutant = self.data.get('df_mutant')
            
            saved_plots = self.visualizer.create_summary_report(
                self.results, figures_folder, 
                series_data=self.series_data,
                real_los=real_los,
                df_mutant_selection=df_mutant
            )
        ...
        if not save_plots:
            ...
            self.visualizer.plot_successful_fits(self.results)
            if self.series_data:
                self.visualizer.visualize_fit_deconvolution(
                    self.results, self.series_data, self.params,
                    real_los=real_los
                )
