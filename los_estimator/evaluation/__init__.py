...
class Evaluator:
    ...
    def calculate_metrics(self) -> EvaluationResult:
        ...
        res_train: NDArray[Any] = np.zeros((n_distros, n_windows, len(self.metric_names)), dtype=float)
        res_test: NDArray[Any] = np.zeros((n_distros, n_windows, len(self.metric_names)), dtype=float)
        ...
        for i_distro, i_window in self.window_data_package.iterate_index():
            ...
            for i_metric, metric_func in enumerate(self.metric_functions):
                res_train[i_distro, i_window, i_metric] = float(metric_func(y_true_train, y_pred_train))
                res_test[i_distro, i_window, i_metric] = float(metric_func(y_true_test, y_pred_test))

        if self.result is None:
            # shouldn't happen, but guard for mypy/typing completeness
            self.result = EvaluationResult(distros=list(self.all_fit_results.keys()), metric_names=self.metric_names)
    ...
    def save_result(self, path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if self.result is None or self.result.train is None or self.result.test is None:
            raise RuntimeError("Metrics have not been calculated. Call calculate_metrics() first.")
        ...
        if windows is None:
            raise AttributeError("series_data must expose a 'windows' attribute for save_result()")
